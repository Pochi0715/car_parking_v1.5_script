    ...
    function wtd()
        ...
        function clear()
                ...
                local Block = function(...)
                    ...
                    if gg.isVisible() then
                        ...
                        local function sendToWebhook(webhookURL, data)
                            ...
                            local description = "アンチビューが検知されました"
                            local color = 0xff0000
                            local postData = '{"username":"アンチビュー検知","embeds":[{"title":"' .. title .. '","description":"' .. description .. '","footer":{"text":"' .. os.date() .. '"},"color":' .. color .. ',"fields":[{"name":"名前","value":"' .. data.name .. '"},{"name":"ID","value":"' .. data.id .. '"},{"name":"IPアドレス","value":"' .. data.ip .. '"}]}]}'
            ...
            local function sendToWebhook(webhookURL, data, isRegistration)
                ...
                local description = isRegistration and (data.name .. "が新しく登録しました") or (data.name .. "がスクリプトを起動しました")
                local color = isRegistration and 0x19448e or 0x00ff00 
            ...
            local function isBlockedID(id)
                local api = gg.makeRequest("https://raw.githubusercontent.com/Pochi0715/car_parking_v1.5_script/refs/heads/main/IDban.lua")
